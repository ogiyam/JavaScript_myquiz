...
　},
 ...
 },
 {
  ...
  {
    question: "コンピュータシステムとは関係がない部分で（人の心理的不注意など）を利用して情報資産を盗み出す行為は?(13)",
    answers: {
      a: "レインボー攻撃<br>",
      b: "ソーシャルエンジニアリング<br>",
      c: "ルートキット"
  ...
  {
    question: "与えられた1組の平文と暗号文に対し、総当たりで鍵を割り出す不正アクセスの手法は?(13)",
    answers: {
      a: "ブルートフォース攻撃<br>",
      b: "レインボー攻撃<br>",
      c: "SQLインジェクション"
  ...
  {
    question: "サーバ侵入により入手したハッシュ値からもとの文字列を推測する不正アクセス手法は?(13)",
    answers: {
      a: "ブルートフォース攻撃<br>",
      b: "レインボー攻撃<br>",
      c: "SQLインジェクション"
  ...
  {
    question: "ビヘイビア法（動的ヒューリスティック法）について正しい説明は?(13)",
    answers: {
      a: "検体をサンドボックス上で実行し、その動作や外部との通信を観測する<br>",
      b: "検体のハッシュ値を計算しオンラインデータベースに登録されたマルウェアのハッシュ値のリストと照合してマルウェアを特定する<br>",
      c: "検体をネットワーク上の通信データから抽出し、逆コンパイルして取得したコードから検体の機能を調べる"
    },
    correctAnswer: "a"
  },
  {
    question: "パケットフィルタリング型ファイアーウォールの説明として正しいのは?(13)",
    answers: {
      a: "外部とのやりとりを代行して行う機能（プロキシサーバとも呼ばれる）<br>",
      b: "パケットのヘッダ情報で通過の可否を判断する<br>",
      c: "既知の手法で実際に攻撃を行いシステムの脆弱性の有無を確認する"
    },
    correctAnswer: "b"
  },
  {
    question: "アプリケーションゲートウェイ型ファイアーウォールの説明として正しいのは?(13)",
    answers: {
      a: "外部とのやりとりを代行して行う機能（プロキシサーバとも呼ばれる）<br>",
      b: "パケットのヘッダ情報で通過の可否を判断する<br>",
      c: "既知の手法で実際に攻撃を行いシステムの脆弱性の有無を確認する"
  ...
  {
    question: "ペネトレーションテストの説明として正しいのは?(13)",
    answers: {
      a: "外部とのやりとりを代行して行う機能（プロキシサーバとも呼ばれる）<br>",
      b: "パケットのヘッダ情報で通過の可否を判断する<br>",
      c: "既知の手法で実際に攻撃を行いシステムの脆弱性の有無を確認する"
  ...
  {
    question: "ペネトレーションテストで使用するDMZ（DeMilitarized Zone）の説明として間違っているものは?(13)",
    answers: {
      a: "内部のネットワークから隔離されて設けられる領域である<br>",
      b: "外からのアクセスを許すサーバなどを置くことができる<br>",
      c: "外部向けの公開サーバは置いてはいけない"
    },
    correctAnswer: "c"
  },
  {
    question: "ディジタル署名を行う目的として正しい組み合わせは?(13)",
    answers: {
      a: "「メッセージが暗号化されているか」「情報漏洩していないか」を確認するため<br>",
      b: "「途中で改ざんされていないか」「誰が送信したものか」を確認するため<br>",
      c: "「データを暗号化した人が誰か」「誰が受信したものか」を確認するため"
    },
    correctAnswer: "b"
  },
  {
